...
Linting (Ruff): Check code style and formatting
Unit Tests (Pytest): Run fast unit tests
Security Scan (Bandit): Detect common security issues
Coverage Report: Ensure test coverage meets threshold
Tools Used:
...
Manual Merge: Human clicks "Merge" button
Auto-Merge: GitHub auto-merges when conditions met
Mergify: Advanced merge automation with custom rules
...
Monitoring: Send metrics to observability platform
...
Fallback: Ensure Ollama is configured for open-source option
CI Failing on Feature Branch
Run locally: ruff check . && pytest tests/
Check for missing dependencies in requirements.txt
Verify Python version matches CI (3.12)
Review workflow logs for specific errors
Deployment Failing
Verify cloud credentials are correct
Check Pulumi state is not locked
Run pulumi preview locally to see changes
Review infrastructure logs in cloud console
Documentation Not Updating
Verify ai-docs-update.yml workflow is enabled
Check if changes were in src/ or infra/ directories
Review workflow permissions (needs write access)
Check AI API rate limits
🤝 Contributing
For Team Members
Follow the feature branch workflow
Let AI assist with PR descriptions
Review AI suggestions critically
Keep documentation updated (AI will help!)
For External Contributors
Fork the repository
Create a feature branch
Follow conventional commit format
Open a PR (AI will assist with description)
